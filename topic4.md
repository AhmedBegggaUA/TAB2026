...
<span style="color:#f88146">**Important**. Note that in AlphaZero there is no Rollout because $v(J_k)$ **replaces the simulation**</span>. 
...
$$
PUCT(s,a) = \underbrace{\frac{W(s,a)}{N(s,a)}}_{Q(s,a)} + C\underbrace{\cdot\pi(a|s)\cdot\frac{\sqrt{N(s)}}{1 + N(a)}}_{U(s,a)}
$$


where **initially** $Q(s,a)=0$, $s$ is the parent and $a$ the child, $N(s,a)$ is the number of times we have applied action $a$ from state $s$ (we can reach the configuration or state $s$ at many different parts of the game, e.g. Tic-Tac-Toe or Chess), $N(s)$ are the visits to $s$ and $N(a)$ are the visits to child $a$. 


<span style="color:#f88146">**Important**. the pair $(s,a)$ refers to **edges** $(parent, child)$. These edges are inialized when the node $s$ is selected so that we have an idea of what the border is</span>. 


In {numref}`PUCT1` se show an example of selection (among all the pairs with a $\text{None}$ child): 


```{figure} ./images/Topic4/PUCT1-removebg-preview.png
---
name: PUCT1
width: 800px
align: center
height: 600px
---
PUCT in AlphaZero (Selection). Credit: [Medium](https://medium.com/@bentou.pub/alphazero-from-scratch-in-pytorch-for-the-game-of-chain-reaction-part-2-b2e7edda14fb).
```


and in {numref}`PUCT2` se show the resulting expansion ($\text{None}$ extremes are created): 


```{figure} ./images/Topic4/PUCT2-removebg-preview.png
---
name: PUCT2
width: 800px
align: center
height: 600px
---
PUCT in AlphaZero (Expansion). Credit: [Medium](https://medium.com/@bentou.pub/alphazero-from-scratch-in-pytorch-for-the-game-of-chain-reaction-part-2-b2e7edda14fb).
```


**Aditional note**. In regular MCTS, $(s,a)$ occurs only once. But note that in a game we can reach **the same state $s$ at different moments of the playing. This is what $N(s,a)$ registers. 
...
<br></br>
<span style="color:#f88146">
**Note**. There are **no rollouts or simulations**. Since initially the weights of the NN are random, AlphaGo plays this way. Later on, with more statistics the NN returns better predictions for the value and the policy. 
</span>
...
#### Testing Tic-Tac-Toe
State of the board after the recommended action:
